...
request and only records counterparts on the digital floor. --fill-gaps
searches gids with no stored edition at all, digital-first with boxed as
fallback, so boxed-only games still gain a FANZA entry.
"""
import csv
import json
import re
...
def main():
    ...
    if not merge_only:
        ...
        if fill_gaps:
            # Games with no stored edition at all: digital-first,
            # boxed-fallback (boxed allowed — that is the point).
            gids = [g for g in sorted(store.keys(), key=int)
                    if not (store.get(g) or {}).get("dmm")
                    and titles.get(g)
                    and not (done.get(g) or {}).get("ok")]
            if only:
            ...
            for i, gid in enumerate(gids):
                ...
                try:
                    done[gid] = enrich_gid(gid, title, None, session, False)
                ...
                print(f"[{i+1}/{len(gids)}] {gid} {title[:20]} "
                      f"ok={r.get('ok')} {(r.get('fill') or {}).get('dmm', '')} "

...
// Python int(float(n)) for the DB's numeric strings ("2" -> 2, "2.5" -> 2,
// non-numeric -> 0; the endpoint only ever sends clean digit strings).
function intFloat(v: unknown): number {
...
// Python int(str(v).strip()) with try/except -> 0 (same as prep_data.ts).
...
const dumpsIndentSorted = (v: PyVal) => encode(v, 1, true, 0);
...
function encode(v: PyVal, indent: number | null, sortKeys: boolean, level: number): string {
  ...
  if (sortKeys) {
    items = items.slice().sort((a, b) => pyStrCmp(a[0], b[0]));
  }
  if (indent === null) {
    // separators=(",", ":")
...
// ---------------------------------------------------------------------------
// RFC4180 CSV (same state machine as prep_data.ts) for csv_ids().
...
export function replaceCharref(g0: string): string {
  ...
  // named charref
  if (Object.prototype.hasOwnProperty.call(HTML5_ENTITIES, g)) return HTML5_ENTITIES[g];
  // find the longest matching name (as defined by the standard)
  for (let x = g.length - 1; x >= 2; x--) {
    const pfx = g.slice(0, x);
    if (Object.prototype.hasOwnProperty.call(HTML5_ENTITIES, pfx)) return HTML5_ENTITIES[pfx] + g.slice(x);
  }
  return "&" + g;
}
...
export function unescapeEntities(s: string): string {
  ...
  // mirror html.unescape: the callback works on group(1), not the full match
  return s.replace(/&(#[0-9]+;?|#[xX][0-9a-fA-F]+;?|[^\t\n\f <&#;]{1,32};?)/g, (_m, g) => replaceCharref(g));
}

// _FirstTable as a regex extractor: the SQL answers are flat tables, so rows
// from <tr>..</tr> and cells <t[dh]>..</t[dh]> suffice; <th> rows are headers.
// <br> inside a cell is a newline; other tags are dropped, text kept.
export function firstTableRows(seg: string): string[][] {
  ...
  while ((m = trRe.exec(seg)) !== null) {
    const cells: string[] = [];
    let th = false;
    const tdRe = /<t([dh])(\s[^>]*)?>([\s\S]*?)<\/t\1>/gi;
    let c: RegExpExecArray | null;
    while ((c = tdRe.exec(m[1])) !== null) {
      if (c[1].toLowerCase() === "h") th = true;
      cells.push(cellText(c[3]));
    }
    if (cells.length && !th) rows.push(cells);
  }
  return rows;
}

...
import { _CACHE, _DATA, _FULLCG, liveCache, USE_GC } from "./00-head.js";
import { dmmEntries, relatedOf, storeOf, tagsOf } from "./10-match.js";
import { playBeep, sessionMetaCache, S } from "./15-state.js";
import { escHtml, getArtworkFor, getMedClass } from "./25-artwork.js";
import { attachStripListeners, coreTitle, ehSiteUrl, fullcgGoogleUrl, groupFor, hitomiSiteUrl, refreshLiveMeta, romajiTitle } from "./35-fullcg-live.js";
import { preloadImageUrl, startRelAutoScroll, stopRelAutoScroll } from "./45-rails-preload.js";
import { triggerVndbFetch } from "./55-vndb-chrome.js";
export function openDetail(gid) {
  ...
  if (storeTabs.length >= 2) {
    if (S.tab === "dmm" && dmmHtml) defaultStoreTab = "dmm";
    else if (S.tab === "dlsite" && dlHtml) defaultStoreTab = "dl";
    else if (S.tab === "getchu" && gcHtml) defaultStoreTab = "gc";
    else if (S.tab === "vndb" && vndbShots.length > 0) defaultStoreTab = "vndb";
  ...
  function createRelCardHtml(r, isClone) {
    var rArt = getArtworkFor(r, S.tab);
  ...
  for (var pi = 0; pi < Math.min(stripImages.length, 2); pi++) {
    preloadImageUrl(stripImages[pi].getAttribute("data-full") || stripImages[pi].getAttribute("src"));
  }

  // Attach Recommendation card click triggers (supports original cards and infinite loop clones)
  dbody.querySelectorAll(".relcard[data-act='detail'], .relclone[data-act='detail']").forEach(function (/** @type {any} */ cardEl) {
